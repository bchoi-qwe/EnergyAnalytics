...
ea_policy_audit_text <- function() {
  ...
  audit_functions <- Filter(
    function(name) exists(name, envir = asNamespace("EnergyAnalytics"), inherits = FALSE),
    c(
      "ea_build_market_registry",
      "ea_build_commodity_curve_long",
      "ea_build_zema_contract_prices_long",
      "ea_build_options_surface_long",
...
test_that("all canonical feather datasets load", {
  ...
  expect_setequal(
    ...
    c(
      ...
      "commodity_contract_prices_long.feather",
...
test_that("curve datasets satisfy canonical schema constraints", {
  ...
  expect_equal(anyDuplicated(contract_prices[, c("date", "market", "source_series")]), 0L)
  ...
  expect_setequal(sort(unique(contract_prices$market)), c("BRN", "CL", "HO", "NG", "RB"))
  expect_equal(sort(unique(ust$market)), "UST")
  expect_equal(length(unique(ust$curve_point)), 11L)
  expect_setequal(sort(unique(expiry$market)), c("BRN", "CL", "HO", "HTT", "NG", "RB"))
  expect_true(all(commodity$source_family == "RTL::dflong"))
  expect_true(all(contract_prices$source_family == "RTL::getPrices"))
})
...
test_that("manifest hashes match current files", {
  ...
  expect_true(length(manifest$datasets) == 9L)
  expect_setequal(manifest$sources$name, c("RTL", "ZEMA", "FRED", "EIA"))
...
test_that("source policy requires the intended live source clients", {
  ...
  expect_true(grepl("RTL::getPrices", audit_text, fixed = TRUE))
...
test_that("source metadata is single-source per row", {
  skip_if_no_snapshot()
  datasets_with_source_family <- c(
    "market_registry",
    "commodity_curve_long",
    "commodity_contract_prices_long",
    "contract_expiry_metadata",
    "ust_curve_long",
    "eia_stocks_long",
    "eia_storage_capacity_long"
  )

  for (dataset in datasets_with_source_family) {
    df <- ea_load_dataset(dataset)
    if ("source_family" %in% names(df)) {
      expect_false(any(grepl(";", df$source_family, fixed = TRUE), na.rm = TRUE))
    }
  }
})
